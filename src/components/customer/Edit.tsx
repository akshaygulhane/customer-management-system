...
class Edit extends React.Component<RouteComponentProps<any>, IFormState>{
    ...
    private formSubmissionHandler = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        e.preventDefault();
        this.setState({ loading: true })

        axios.patch(`http://localhost:5000/customers/${this.state.id}`, this.state.values)
            .then(response => {
                if (response.data) {
    ...
    private setValues = (values: IValues) => {
        this.setState({ values: { ...this.state.values, ...values } } as any);
    }
